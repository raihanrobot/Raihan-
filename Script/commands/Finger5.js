...
module.exports.config = {
  name: "finger5", //⚠️ 𝗗𝗼𝗻'𝘁 𝗖𝗵𝗮𝗻𝗴𝗲 𝗡𝗮𝗺𝗲 — 𝗖𝗺𝗱 𝗪𝗶𝗹𝗹 𝗡𝗼𝘁 𝗪𝗼𝗿𝗸✅
  version: "2.0",
  hasPermssion: 2,
...
async function generateFrameWithFallback({ senderID, mention, credit, apiList }) {
 ...
 for (const baseApi of apiList) {
    ...
    const apiUrl = `${cleanBase}/api/frame?type=finger5&senderId=${senderID}&mentionId=${mention}&credit=${encodeURIComponent(credit)}`;
  try {
...
module.exports.run = async function ({ api, event, args }) {
 ...
 const apiConfigList = await getApiList(module.exports.config.name);
    ...
    const outPath = path.join(__dirname, `finger5_${Date.now()}.png`);
